// https://github.com/tetherto/lib-wallet/blob/main/example/node/cli.js

const repl = require('repl');
const fs = require('fs');
const config = require('./config.json');
const createWallet = require('lib-wallet/src/wallet-lib.js');
const { tetherBasePath } = require('../helpers/utils.js');
...
async function main() {
  const configFile = `${tetherBasePath}/config.json`;
...
function startcli(wallet) {
  ...
  const commands = [
        ...
        for (const method of wallet.pay) {
          ...
          if (tokens.size > 0) {
            for (const [tName, token] of tokens) {
              console.log(
                `    Token: ${tName} - ${token.getTokenInfo().contractAddress}`
              );
            }
          }
          console.log('---');
    ...
    [
      ...
      async (args) => {
        const { token, name, address, sender, amt, err, fee } = parseArgs(
          args,
          wallet
        );
        if (err) return;
        const opts = {};
        if (token) opts.token = token;
        const tx = await wallet.pay[name].sendTransaction(opts, {
          address,
          amount: amt,
          unit: 'main',
          sender,
